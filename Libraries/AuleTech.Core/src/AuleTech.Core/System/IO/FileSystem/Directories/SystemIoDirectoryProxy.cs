...
internal partial class SystemIoDirectoryProxy : ISystemIoDirectory
{
    ...
    {
        return Directory.Exists((PlatformPathStringStandard)folder);
    }
    ...
    {
        return searchPatterns.SelectMany(pattern => GetFiles(path, pattern))
            .Distinct()
            .ToArray();
    }
    ...
    public string?[] GetFiles(string path
        ...
        , SearchOption searchOption)
    {
        return searchPatterns.SelectMany(pattern => GetFiles(path, pattern, searchOption))
            .Distinct()
            .ToArray();
    }
    ...
    public string[] GetDirectories(string folderPath
        , SearchOption searchOption)
    {
        return GetDirectories(folderPath, "*", searchOption);
    }
        ...
        , string destination)
    {
        ...
        var tasks = files.Select(async file =>
        {
            ...
            var destinationFilePath =
                (string)(PlatformPathStringStandard)Path.Combine(destination, sourceFileRelativePath);
