...
internal partial class SystemIoDirectoryProxy
{
    private class DirectoryUnlocker
    {
            ...
            , CancellationToken cancellationToken)
        {
            ...
            string ResolveUnlockerFolderPath()
            {
                return SystemIoProxy.Default.Path.GetPlatformTempPath("unlocker");
            }

            string ResolveHandleExePath()
            {
                return SystemIoProxy.Default.Path.Combine(ResolveUnlockerFolderPath(), "handle.exe");
            }
        }
            ...
            , CancellationToken cancellationToken)
        {
            ...
            async Task<bool> IsLsofInstalledAsync()
            {
                return (await RunBashAsync("which", "lsof")).ExitCode == 0;
            }
