...
namespace AuleTech.Core.Processing.Runners
{
	internal class CommandLineProcessRunner : IProcessRunner
    {
		...
		public ProcessResult RunBash(PlatformProcessStartInfo startInfo
		                             , CancellationToken cancellationToken = default
		                             , bool appendOutputPrefix = true) =>
			RunBashAsync(startInfo, cancellationToken, appendOutputPrefix)
				.GetAwaiter()
				.GetResult();
		...
		public ProcessResult Run(PlatformProcessStartInfo startInfo
		                         , CancellationToken cancellationToken = default
		                         , bool appendOutputPrefix = true) =>
			RunAsync(startInfo, cancellationToken, appendOutputPrefix)
				.GetAwaiter()
				.GetResult();
		...
		public ProcessResult Run(string process
		                         ...
		                         , bool appendOutputPrefix = true
		                         , CancellationToken cancellationToken = default) =>
			Run(new PlatformProcessStartInfo(process, arguments), cancellationToken, appendOutputPrefix);

		public async Task<ProcessResult> RunAsync(PlatformProcessStartInfo startInfo
		                                          , CancellationToken cancellationToken = default
		                                          , bool appendOutputPrefix = true) =>
			await ExecuteAsync(startInfo, cancellationToken, appendOutputPrefix);
